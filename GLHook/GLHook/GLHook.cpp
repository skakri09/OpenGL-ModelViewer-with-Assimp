...
DWORD HookGeneralFunction(const char *Dll, const char *FuncName, void *Function, unsigned char *backup)
{
	...
	ReadProcessMemory((HANDLE)-1, (void*)addr, backup, 6, 0);					// Read the first 6 Bytes of the API and save them
    DWORD calc = ((DWORD)Function - addr - 5);											// Calculate the jump
	memcpy(&jmp[1], &calc, 4);															//build the jmp
	//memcpy((void*)addr, &jmp, 6);
	WriteProcessMemory((HANDLE)-1, (void*)addr, jmp, 6, 0);					//Overwrite the first 6 Bytes of the API with a jump to our function
