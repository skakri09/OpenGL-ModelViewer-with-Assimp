...
glm::vec3 VirtualTrackball::getClosestPointOnUnitSphere(int x, int y) 
{
	...
	if(k >= 0.5f)
	{
		point_on_sphere = glm::vec3(normalized_coords.x/k, normalized_coords.y/k, 0);
		//std::cout << "outside," << k << std::endl;
	}
	else
	{
		point_on_sphere = glm::vec3(2.0f*normalized_coords.x, 2.0f*normalized_coords.y, sqrt(1-( 4*pow(k, 2))));
